...
def timeline():
    """Shows a users timeline or redirect to the splash page.
       ...
       Shows photos by the user and his/her followers
    """
    if not g.user:
    ...
    # We want to return the user's photos as well
    follower_ids.append(user_id)

    photos = db.session.query(Photo).filter(Photo.user_id.in_(follower_ids))
    photos = photos.order_by(Photo.pub_date.desc())
    photos = photos.limit(PER_PAGE)
    return render_template('timeline.html', photos=photos.all())
...
def user_timeline(username):
    ...
    photos = profile_user.photos.order_by(Photo.pub_date.desc()).limit(PER_PAGE)
...
def create_photo(user, path, desc, pub_date=None):
    ...
    photo = Photo(user, path, desc, pub_date)
